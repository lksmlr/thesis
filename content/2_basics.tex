...
Dieses vorgeschaltete System ist nicht Teil der OCR/YOLO-Pipeline und somit kein Bestandteil dieser Arbeit.
Wenn ein Dokument als PDF vorliegt, extrahiert das System den Text mithilfe des Textlayers und eines \gls{OCR}-Modells.
Bei einer Bilddatei kommt lediglich das \gls{OCR}-Modell zum Einsatz.
Die Ergebnisse sind eine PDF/A sowie der \gls{OCR}-Text.

Auf Basis des extrahierten \gls{OCR}-Textes führt die Pipeline in der nächsten Stufe zunächst eine Klassifikation durch.
...
\begin{enumerate}
    ...
    \item \textbf{Datenbereitstellung:} Die Pipeline lädt sowohl das VLM als auch die Dokumente aus dem S3-Store in den \gls{GPU}-Cluster.
...
\begin{table}[ht]
    \centering
    \begin{tabularx}{\textwidth}{Xccc} % 'X' nimmt den restlichen Platz ein
        \hline
