...
Mit einem statischen \gls{VRAM}-Bedarf von circa 17 GB lässt sich dieses Modell auf gängigen Enterprise-GPUs betreiben, ohne die hohen Hardwareanforderungen des 32B-Modells zu stellen.
...
Neben der Modellleistung stellt die architektonische Komplexität einen zentralen Faktor für den produktiven Einsatz dar.
Zur systematischen Einordnung wird im Folgenden das Qualitätsmodell der ISO/IEC~25010 herangezogen, insbesondere das Qualitätsmerkmal der Wartbarkeit mit den Teilaspekten Modularität, Analysierbarkeit, Änderbarkeit und Testbarkeit.

Die Architektur der YOLO/OCR-Pipeline basiert auf einer sequenziellen Verkettung mehrerer spezialisierter Modelle (siehe Abbildung~\ref{fig:aube}).
Zwar ist die Pipeline modular aufgebaut, jedoch besteht eine starke Kopplung zwischen den Komponenten.
Im Speziellen nimmt der Klassifikator eine zentrale Funktion ein, da dessen Entscheidung die Auswahl der nachgelagerten Modelle bestimmt.
Fehlklassifikationen propagieren somit durch die gesamte Modellkette, wodurch die Analysierbarkeit und Fehlereingrenzung erschwert wird.
...
Auch im Hinblick auf die Änderbarkeit zeigt sich eine erhöhte Komplexität.
Eine neue Dokumentenart erfordert Anpassungen am Klassifikator sowie das Training zusätzlicher \gls{YOLO}-Modelle.
Das Training dieser Modelle ist mit großem manuellem Aufwand verbunden, da Bounding-Box-Annotationen erstellt werden müssen.
Darüber hinaus erhöhen die zahlreichen Schnittstellen zwischen den Modulen sowie notwendige Konvertierungslogiken die Fehleranfälligkeit und reduzieren die Modularität im Sinne der ISO/IEC~25010.

Im Gegensatz dazu verfolgt ein \gls{VLM} einen Ansatz, bei dem Klassifikation und Information Extraction innerhalb eines einzelnen Modells erfolgen.
Dadurch reduziert sich die Anzahl der Komponenten und Schnittstellen signifikant, was sich positiv auf die Modularität und Analysierbarkeit auswirkt.
Änderungen an Dokumentenarten beschränken sich bei angepassten Modellen, wie dem Qwen-2.5-VL-7B-finetuned, auf ein erneutes Training eines einzigen Modells.
Bei Basismodellen können Erweiterungen durch Prompt-Anpassungen realisiert werden, wodurch sich der Änderungsaufwand weiter reduziert.

Ein potenzieller Nachteil des \gls{VLM}-Ansatzes liegt in der stärkeren Zentralisierung der Verarbeitung, wodurch eine erhöhte Abhängigkeit von einem einzelnen Modell entsteht.
Zudem weisen \glspl{VLM} eine höhere Varianz in ihren Ausgabeformaten auf, was zusätzliche Normalisierungsschritte erforderlich machen kann und die Testbarkeit beeinflusst.
