...
Die Methodik gliedert sich in vier Phasen, die farblich markiert sind.
...
Jeder Comparator besitzt die Funktion \texttt{compare}, die den vorhergesagten Wert und den korrekten Wert übergeben bekommt.
...
Um die verschiedenen Komparatoren den Feldern zuzuordnen, existiert für jede Dokumentenart eine Konfiguration, die dem jeweiligen \gls{JSON}-Schema aus Abschnitt~\ref{subsec:documents_as_json} entspricht.
...
Diese Struktur ermöglicht es dem Evaluations-Skript, über alle generierten Felder zu iterieren und die passgenaue Vergleichslogik anzuwenden.
...
Da das Feld \texttt{type} in jedem \gls{JSON}-Objekt der Dokumentenarten (siehe Listings~\ref{lst:json_kg5b},~\ref{lst:json_vertrag} und~\ref{lst:json_sonstiges}) vorkommt, kann es nicht halluziniert werden.
...
Diese Parameter bieten einen Kompromiss aus Anpassungsfähigkeit des Modells und vertretbarem VRAM-Bedarf.
Zusätzlich wird durch \texttt{use\_rslora=True} die rsLoRA-Variante aktiviert.
...
Um ein Overfitting zu vermeiden, wird neben dem Trainingsdatensatz auch hier der Validierungsdatensatz verwendet.
