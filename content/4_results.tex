...
Die gemeinsame Betrachtung der F1-Scores und den Confusion Matrices belegt, dass das Fine-Tuning die Klassifikationsleistung steigert.
Ein spezifisches Training schafft in der Klassifikation einen Vorteil gegenüber eines größeren Basismodells.
...
Abbildung~\ref{fig:results_latency_peft_32b_7b} stellt die durchschnittlichen und maximalen \gls{Latenz}en des Qwen-2.5-VL-32B und Qwen-2.5-VL-7B-finetuned dar.

Es zeigt sich ein deutlicher Vorteil des kleineren Modells auf.
Das Qwen-2.5-VL-7B-finetuned antwortet nach durchschnittlichen 9,57 Sekunden, während das Qwen-2.5-VL-32B mit 64,16 Sekunden mehr als das sechsfache an Zeit benötigt.
Bei den Maximalwerten wird der Unterschied noch signifikanter, da hier das größere Modell bis zu 252,06 Sekunden benötigt.
Die maximale Antwortzeit des trainierten 7B-Modells liegt mit 21,6 Sekunden nur etwas über dem Durchschnittswert.

Die Messwerte verdeutlichen, dass ein Fine-Tuning keinen deutlichen Einfluss auf die Inferenzzeit gegenüber dessen Basismodell hat.
Mit der höheren Parameteranzahl benötigt das 32B-Modell länger zum Antworten, was das große Modell in seiner Benutzung einschränkt.
...
In Abbildung~\ref{fig:results_vram_model} wird die statische VRAM-Nutzung der VLMs sowie der OCR/YOLO-Pipeline bei der Initialisierung in Gigabyte visualisiert.

Die OCR/YOLO-Pipeline verzeichnet trotz der Verwendung mehrerer Modelle einen geringen statischen Verbrauch von 4,21 GB.
Die beiden 7B-Modelle liegen mit 16,40 GB (Qwen-2.5-VL-7B) beziehungsweise 16,88 GB (Qwen-2.5-VL-7B-finetuned) auf einem Level.
Den mit Abstand höchsten Verbrauch weist das Qwen-2.5-VL-32B auf, dass mit 64,18 GB den VRAM-Verbrauch der Pipeline um mehr als das 15-fache übersteigt.
Zwischen den drei Qwen-Modellen reiht sich das Pixtral-12B mit einem Verbrauch von 23,89 GB ein.
...
Auch in dieser Messung erweist sich die OCR/YOLO-Pipeline mit einer durchschnittlichen Auslastung von 0,49 GB.
Der maximale Bedarf liegt mit 1,02 GB unter dem Durchschnittsverbrauch des Qwen-2.5-VL-7B (1,97 GB).
Auffällig ist das Verhältnis des Qwen-2.5-VL-7B-finetuned und des größeren Qwen-2.5-VL-32B.
Das kleinere, trainierte Modell hat mit durchschnittlich 3,03 GB und maximal 6,36 GB einen geringfügig höheren Verbrauch als das 32B-Modell.

Die gemeinsame Betrachtung der statischen und dynamischen VRAM-Auslastung verdeutlicht den Architekturunterschied der Pipeline und den VLMs.
Selbst das kleinste Modell ist im Gegensatz zu der OCR/YOLO-Pipeline um ein Vielfaches größer, was einen Unterschied in der Hardware zur Laufzeit darstellt.
