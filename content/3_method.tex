...
Um die Eignung der VLMs als potenziellen Ersatz für die OCR/YOLO-Pipeline zu beurteilen, beantwortet die Arbeit folgende Forschungsfragen:

\begin{enumerate}
    \item Wie verhält sich ein nicht domänenspezifisch angepasstes VLM hinsichtlich \gls{Latenz}, Klassifikations- und Extraktionsleistung im Vergleich zur OCR/YOLO-Pipeline, und welche Vorteile bietet ein einzelnes generalistisches Modell gegenüber spezialisierten Einzelsystemen?
    ...
    \item Wie unterscheidet sich der Ressourcenverbrauch (Energieverbrauch und \gls{VRAM}"=Auslastung) der VLMs von dem der OCR/YOLO-Pipeline?
\end{enumerate}


\section{Überblick über die Methodik}\label{sec:overview}

Abbildung~\ref{fig:overview} gibt einen Gesamtüberblick der Methodik.
Die Methodik gliedert sich in vier Phasen, die farblich getrennt sind.
Ausgehend vom Dokumentenbestand erfolgt zunächst ein Preprocessing der Daten, bevor aus einem Teil der Dokumente ein Test- und Validierungsdatensatz erstellt wird.
Im Anschluss wird in einem iterativen Prozess mithilfe von Prompt-Refinement und Fehleranalysen das am besten geeignete Basismodell ausgewählt.
Dieses dient zusammen mit dem Trainingsdatensatz als Grundlage für das Fine-Tuning.
Die Anpassung der Parameter des Fine-Tunings basiert ebenfalls auf einem iterativen Prozess der Fehleranalyse.
Die finalen Metriken werden mithilfe des Testdatensatzes bestimmt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/overview}
    \caption{Überblick über die Methodik}
    \label{fig:overview}
\end{figure}
...
Durch Datenlieferungen der Familienkasse stehen für die Evaluation mehrere Tausend Dokumente als PDFs oder Bilddateien zur Verfügung.
...
Dieser Prozess ist im Überblick der Methodik~\ref{fig:overview} in Rot dargestellt.
...
Zur Korrektur der Rotation werden die Exchangeable Image File Format (EXIF)-Metadaten ausgelesen, um die ursprüngliche Ausrichtung wiederherzustellen.
Während ältere VLMs Bilder auf eine feste Dimension skalieren, unterstützen die in dieser Arbeit evaluierten Modelle eine dynamische Skalierung.
...
Listing~\ref{lst:preprocessing} zeigt einen Teil des Preprocessings in Python.
Mithilfe der Bibliothek \texttt{Pillow} (PIL) wird jedes Dokumentenbild mit der Funktion \texttt{ImageOps.exif\_transpose()} korrigiert.
Im Anschluss wird, wenn das Bild die maximale Pixelanzahl überschreitet, ein Skalierungsfaktor ermittelt.
Mit diesem Faktor wird das Bild dann verkleinert.
...
Da moderne VLMs darauf trainiert sind, Antworten im JavaScript Object Notation (\gls{JSON})-Format zu liefern, werden die extrahierten Informationen in ein vordefiniertes \gls{JSON}-Schema überführt.
Die \gls{JSON}-Schemata für die relevanten Dokumentenarten sind in den Listings~\ref{lst:json_vertrag} und~\ref{lst:json_kg5b} dargestellt.
Im \gls{JSON}-Schema der sonstigen Dokumente in Listing~\ref{lst:json_sonstiges} findet nur die Klassifikation statt, da hier keine Informationen relevant sind.
...
Felder der Information Extraction sind über die Dokumentenarten hinweg nicht konsistent, jedoch bildet das Schema eines Vertrags eine Teilmenge des Schemas eines KG5b-Formulars.
Das Schema des KG5b-Formulars beinhaltet zusätzlich die Felder \texttt{apprenticeship\_finished} und \texttt{exam\_month}.
Das Feld \texttt{apprenticeship\_finished} bildet die Checkboxen auf der ersten Seite des Formulars ab (siehe Anhang~\ref{fig:kg5b_page_1}).
...
Mehrseitige Dokumente werden durch ein \gls{JSON}-Objekt repräsentiert.
...
Die sonstigen Dokumente beinhalten keine kontextfremden Exemplare, sondern Schulbescheinigungen, Studienbescheinigungen und Eintragungen bei der Handwerkskammer, um die Komplexität der Unterscheidung zu erhöhen.

Zur Erstellung der Ground Truth werden alle Dokumente manuell annotiert und in das entsprechende \gls{JSON}-Schema überführt.
...
Um eine korrekte Formatierung des \gls{JSON}-Outputs der VLMs sicherzustellen, existieren verschiedene Ansätze.
...
Schlägt das Parsen der \gls{JSON}-Objekte fehl, werden diese mit der Anweisung, sie zu korrigieren, an das VLM zurückgeliefert.
...
\newpage
\subsection{Vergleichslogiken der unterschiedlichen JSON-Felder}\label{subsec:comparator}

Im Gegensatz zur Verarbeitung von Antworten herkömmlicher Modelle weisen VLMs eine hohe Varianz an Ausgabeformaten auf.
...
Jeder Comparator implementiert die Funktion \texttt{compare}, die den vorhergesagten Wert und den korrekten Wert übergeben bekommt.
...
Der \texttt{NameComparator} in Listing~\ref{lst:name_comparator} vergleicht die Namensfelder mithilfe der \gls{Levenshtein-Similarity}, um kleinere Fehler und verschiedene Schreibweisen zu tolerieren.
...
Da die OCR/YOLO-Pipeline denselben Vergleich mit identischem Threshold verwendet, wird eine Vergleichbarkeit der Ergebnisse gewährleistet.
Um die \gls{Levenshtein-Similarity} zu ermitteln, wird mit der Bibliothek \texttt{rapidfuzz} die Levenshtein-Distanz bestimmt und anschließend durch Division durch 100 normalisiert.
...
Zu den akzeptierten Varianten gehören sowohl \texttt{true} und \texttt{false}, \texttt{0} und \texttt{1}, \texttt{wahr} und \texttt{falsch} sowie \texttt{vorhanden} und \texttt{nicht vorhanden}.
...
Um die verschiedenen Komparatoren den Feldern zuzuordnen, existiert für jede Dokumentenart eine Konfiguration, die dem jeweiligen \gls{JSON}-Schema entspricht.
Listing~\ref{lst:scheme_kg5b} zeigt beispielhaft die Konfiguration für das KG5b-Formular.
Hierbei wird über ein Python-Dictionary jedem erwarteten Feldnamen sowohl der Datentyp als auch der spezifische Comparator zugewiesen.
Diese Struktur ermöglicht es dem Evaluations-Skript, über alle generierten Felder zu iterieren und die passende Vergleichslogik anzuwenden.
...
Um die generierten \gls{JSON}-Objekte der VLMs schlussendlich bewerten zu können, werden die Klassifikation und Information Extraction unabhängig voneinander evaluiert.
Für die Evaluation werden die einzelnen \gls{JSON}-Felder jedes Dokuments mithilfe der in der Konfiguration definierten Komparatoren mit der Ground Truth verglichen und verschiedene Metadaten erfasst.
...
Halluzinierte Felder sind definiert als Felder, die nicht in der jeweiligen Konfiguration beziehungsweise im Schema vorkommen.
...
Da das Feld \texttt{type} in jedem \gls{JSON}-Objekt der Dokumentenarten vorkommt, kann es nicht halluziniert werden.
...
Zu den \gls{FN} gehören Felder, die entweder fehlen oder vorhanden sind, aber falsch erkannt wurden.
...
Durch die Schemata und die jeweilige Ground Truth ist eindeutig feststellbar, welche Felder halluziniert sind.
Da mithilfe der Schemata auch ohne Ground Truth die benötigten Felder gefiltert werden können, lässt sich die Strenge der Metrik steuern.
Listing~\ref{lst:penalize_hallucinations} zeigt einen Ausschnitt der Evaluationslogik.
Über den Parameter \texttt{penalize\_hallucinations} kann entschieden werden, ob Halluzinationen in die Berechnung einfließen.
Ist der Parameter auf \texttt{True} gesetzt, wird die Anzahl der halluzinierten Felder zu den False Positives addiert, was den finalen F1-Score beeinflusst.

\begin{listing}[H]
    \caption{Parameter, der bestimmt, ob Halluzinationen bestraft werden}
    \label{lst:penalize_hallucinations}
    \inputminted[]{python}{listings/penalize_hallucinations.py}
\end{listing}
...
Mit der Initialisierung des Modells wird zudem einmalig der statische Verbrauch des VLMs erfasst.
...
Wie im Überblick der Methodik~\ref{fig:overview} (grün markierte Felder) dargestellt, erfolgt diese Auswahl in einem iterativen Prozess.
...
In jeder Iteration werden die Modelle Pixtral-12B und Qwen-2.5-VL-7B mit dem Validierungsdatensatz evaluiert.
...
\begin{enumerate}
    \item \textbf{Evaluation}: Durchführung der Inferenz beider Modelle mit dem Validierungsdatensatz.
    \item \textbf{Fehleranalyse}: Untersuchung der Fehler in der Klassifikation und in der Information Extraction.
    \item \textbf{Prompt-Refinement}: Anpassung des Prompts auf Basis der Fehleranalyse.
\end{enumerate}
...
Im Anschluss werden mit dem finalen Prompt und dem Testdatensatz die finalen Metriken bestimmt und auf deren Basis das Modell gewählt.
Die Trennung von Validierungs- und Testdatensatz schließt ein Data Leakage aus und verhindert ein Overfitting auf den Testdaten.

Bezogen auf das \gls{Prompt Engineering} wird bewusst ein \gls{Zero-Shot Prompting} gewählt.
Im Gegensatz zu \gls{One-Shot Prompting} und \gls{Few-Shot Prompting} werden hier weniger Tokens verbraucht, was die Kosten pro Inferenz senkt.
Des Weiteren kann das Modell so schneller antworten, da es weniger Input-Tokens verarbeiten muss.

Die Gestaltung des finalen Prompts (siehe Listing~\ref{lst:prompt}) folgt der \gls{R-K-F-Formel}.
...
Um ein qualitativ hochwertiges Fine-Tuning zu ermöglichen, ist ein umfangreicherer Datensatz als für die Modellauswahl erforderlich.
...
Hierbei generiert das im Prozess der Modellauswahl gewonnene Modell, zusammen mit dem angepassten Prompt, für jedes Dokument das jeweilige \gls{JSON}-Objekt.
Im Anschluss werden diese \gls{JSON}-Objekte manuell kontrolliert und korrigiert.
Durch den Einsatz des Modells zur Ermittlung der Ground Truth kann der Aufwand gesenkt werden.

Für das Fine-Tuning muss der Datensatz, wie in Listing~\ref{lst:messages} dargestellt, in ein bestimmtes Format gebracht werden.
Zu Beginn werden die \gls{JSON}-Objekte in Markdown-Blöcke eingebettet, um dem bereits gelernten Format des VLM zu entsprechen.
Diese Blöcke werden dann in das \gls{OpenAI Message Format} eingefügt.
Die PIL-Images der Dokumente werden einzeln in den Kontext des Benutzers mit \texttt{\char`\"type\char`\": \char`\"image\char`\"} eingebunden.
...
Das in Abschnitt~\ref{sec:model_selection} gewählte Basismodell wird mittels PEFT an die Dokumentenarten angepasst.
...
Der Trainingsdatensatz wird mithilfe von Datenaugmentation an das Training angepasst.
...
Die Parameteroptimierung erfolgt iterativ, wie im blau markierten Bereich in Abbildung~\ref{fig:overview} zu erkennen ist.
Wie Listing~\ref{lst:parameter} verdeutlicht, werden alle Komponenten des VLM trainiert (\texttt{finetune\_...=True}).
Für die LoRA-Konfiguration wurde ein Rang von $r=16$ sowie ein Alpha-Wert (\texttt{lora\_alpha}) von 16 gewählt.
Diese Parameter bieten einen Kompromiss aus Anpassungsfähigkeit des Modells und \gls{VRAM}-Auslastung.
Zusätzlich wird durch \texttt{use\_rslora=True} die rsLoRA-Variante des PEFT aktiviert.

Die Steuerung des eigentlichen Trainings erfolgt über den \texttt{SFTTrainer}.
Um Speicherplatz zu sparen und die Berechnungen zu beschleunigen, wird der 8-Bit-AdamW-Optimizer in Kombination mit einer Learning Rate von $5 \cdot 10^{-5}$ und einem Cosine-Scheduler eingesetzt.
Die maximale Sequenzlänge (\texttt{max\_seq\_length}) wird auf 20.000 Token beschränkt, um keine Out-of-Memory-Fehler zu erhalten.
Der Parameter \texttt{neftune\_noise\_alpha=15} fügt ein geringes Rauschen in die Trainingsdaten ein, um einem Overfitting entgegenzuwirken.

Um zusätzlich ein Overfitting zu vermeiden, wird neben dem Trainingsdatensatz auch der Validierungsdatensatz verwendet.
...
Abschließend werden, wie bei der Auswahl des Basismodells, mit dem Testdatensatz die finalen Metriken bestimmt.
Die trainierten LoRA-Adapter werden zur Laufzeit geladen, um Speicherplatz zu sparen, da sonst das Qwen-2.5-VL-7B zusammen mit jedem Adapter gespeichert werden müsste.