...
Somit sind die Testdaten unabhängig von den Daten mit denen der prompt angepasst wurde.
...
Durch den Einsatz des Modells zur Ermittelung der Ground Truth konnte der Aufwand gesenkt werden.


\section{Fine-Tuning}\label{sec:fine_tuning}

Nach der Auswahl des Basismodells folgt ein Parameter-Efficient Fine-Tuning (\gls{PEFT}) dieses Modells.
Das Ziel ist eine Anpassung an die vorliegenden Dokumentenarten sowie deren \gls{JSON}-Objekte.

Für das Training wird der Trainingsdatensatz~\ref{subsec:train_dataset} verwendet.
Zur Durchführung wird das Framework Unsloth benutzt.