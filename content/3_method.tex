...
Im Rahmen dieses Proof of Concept werden verschiedene \glspl{VLM} evaluiert und der bestehenden \gls{YOLO}/\gls{OCR}-Pipeline gegenübergestellt.
...
\subsection{Modellierung der Dokumentenarten als JSON-Objekte}\label{sec:documents_as_json}
...
Während komplexere Lösungen nur Tokens zulassen die syntaktisch in \gls{JSON}-Objekten enthalten sein könnten, wird in dieser Evaluation ein direkterer Ansatz gewählt.
Hierbei werden aus der generierten Antwort potenzielle \gls{JSON}-Objekte extrahiertrt und geparst.
...
Des Weiteren wird das Modell in 4-Bit geladen, um die \gls{VRAM}-Auslastung weiter zu senken.
...
Darüber hinaus wurden in einigen der Trainingsläufe die Seiten der Dokumente wahllos getauscht, um dem Modell anzutrainieren, Informationen unabhängig von der Position zu extrahieren.
...
Am Ende des Trainingslaufs wird das Modell geladen, dass den geringsten Loss gegenüber dem Validierungsdatensatz hatte.
