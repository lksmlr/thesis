...
Mithilfe von Parameter-Efficient Fine-Tuning (\gls{PEFT}) kann die Anzahl der zu trainierenden oder neuen Parameter gesenkt werden.
...
Das automatisierte Lesen, Verstehen und Analysieren von Dokument wird ein immer relevanteres Forschungsfeld und wird häufig unter dem Begriff DocumentAI~\cite{DocumentAI} zusammengefasst.

Begonnen hat die Entwicklung mit
...
\subsection{Deep Learning mit OCR-Modell}\label{subsec:deeplearning_ocr}



\subsection{Transformer mit OCR-Modell}\label{subsec:tranformer_ocr}

\subsection{End-to-End Transformer}\label{subsec:end_to_end_transformer}