...
\section{Überblick der Methodik}\label{sec:overview}

Abbildung~\ref{fig:overview} skizziert einen Gesamtüberblick der Methodik.
...
Das binäre Feld \texttt{apprenticeship\_finished} bildet die Checkboxen auf der ersten Seite des Formulars ab (siehe Anhang~\ref{fig:kg5b_page_1}).
...
Um die \gls{Levenshtein-Similarity} zu ermitteln, wird mit der Bibliothek \texttt{rapidfuzz} die Levenshtein-Distanz bestimmt und anschließend durch Division mit 100 normalisiert.
...
Das Code-Beispiel in Listing~\ref{lst:scheme_kg5b} definiert beispielhaft die Konfiguration für das KG5b-Formular.
...
Um die generierten \gls{JSON}-Objekte der VLMs schlussendlich bewerten zu können, werden die Klassifikation und Information Extraction unabhängig voneinander bewertet.
...
Im Anschluss werden mit dem finalen Prompt und dem Testdatensatz~\ref{subsec:test_val_dataset} die finalen Metriken bestimmt und auf deren Basis das Modell gewählt.
...
Die PIL-Images der Dokumente werden einzeln in den Kontext des Benutzers mit \texttt{"type": "image"} eingebunden.
