...
Zusätzlich laden Kunden häufig weitere Unterlagen im Portal hoch.
...
\begin{figure}[H]
    ...
    \caption{Schematischer Ablauf der aktuellen OCR/YOLO-Pipeline zur Dokumentenverarbeitung}
    \label{fig:aube}
\end{figure}

Der schematische Ablauf des Ist-Zustands ist in Abbildung~\ref{fig:aube} dargestellt und lässt sich in ein vorgeschaltetes System und die OCR/YOLO-Pipeline einteilen.

Dieses vorgeschaltete System ist nicht Teil der Arbeit, jedoch der Ausgangspunkt für die Verarbeitung.
Es liefert als Ergebnis ein PDF/A sowie den daraus extrahierten \gls{OCR}-Text.

Auf Basis des extrahierten \gls{OCR}-Textes führt die Pipeline zunächst eine Klassifikation durch.
...
Handelt es sich hingegen um ein KG5b-Formular oder einen Vertrag, startet das System je nach Dokumententyp eine Erkennung mit einem spezifischen \gls{YOLO}-Modell.
Innerhalb der Bounding-Boxen extrahiert die Pipeline mit einem \gls{OCR}-Modell den Text und ordnet ihn dem jeweiligen Label zu.

Abschließend stehen die erkannten Informationen zur weiteren Verarbeitung bereit.
...
Für die rechenintensiven Aufgaben, insbesondere das Fine-Tuning und die Inferenz der VLMs, stehen innerhalb des Clusters zwei NVIDIA A40 \glspl{GPU} mit jeweils 48 GB \gls{VRAM}~\cite{NVIDIAA40} zur Verfügung.
Um die Bereitstellung der VLMs zu erleichtern, nutzt das System Kubeflow.
Kubeflow ist eine Open-Source-Plattform, die speziell für das Entwickeln, Trainieren und Deployen von Machine-Learning-Modellen konzipiert wurde~\cite{Kubeflow}.

Der Zugang zum Cluster sowie zur Integrated Development Environment (\gls{IDE}) erfolgt über eine noVNC-Schnittstelle (browserbasierter Remote-Desktop)~\cite{noVNC}.
...
\begin{enumerate}
    ...
    \item \textbf{Datenbereitstellung:} Die Pipeline lädt sowohl das VLM als auch die Dokumente aus dem S3-Store in den Kubeflow-Job.
    \item \textbf{Verarbeitung:} Die Inferenz oder das Training findet im Kubernetes-Cluster auf den bereitgestellten \glspl{GPU} statt.
    \item \textbf{Persistierung:} Der Kubeflow-Job lädt die Ergebnisse zurück in den S3-Store.
\end{enumerate}
...
Der Encoder überführt jeden Patch in einen Vektor, der die Eigenschaften des Ausschnitts repräsentiert.
Das Ergebnis ist eine Sequenz von Bildvektoren, die die visuellen Features des gesamten Bildes beinhaltet.
...
Der Adapter projiziert die Bildvektoren des Image-Encoders in denselben Vektorraum wie die Textvektoren des LLMs.
...
Dabei ergibt sich der statische Verbrauch aus den Gewichten der Modelle, während sich der dynamische Verbrauch aus den Key-Value-Caches und den Aktivierungen zusammensetzt.
...
Eine Besonderheit dieser Modelle ist, dass sie speziell auf die Verarbeitung von Dokumenten trainiert wurden~\cite{QwenPaper}.
...
Bei Standard-LoRA führt ein steigender Rang $r$ oft nicht zu einer besseren Leistung, da der verwendete Faktor ($\alpha/r$) das Lernen bei höheren Rängen verlangsamen oder hemmen kann.
...
Cui et al.\ liefern eine Übersicht über die Entwicklung dieses Bereichs und definieren den Begriff Document AI als Prozess, der Webseiten, digitale und gescannte Dokumente in strukturierte Informationen überführt~\cite{DocumentAI}.
...
Durch den Einsatz von Convolutional Neural Networks auf dieser Struktur konnte das Modell sowohl textuelle als auch räumliche Merkmale nutzen, was zu einer Leistungssteigerung bei Aufgaben wie der Informationsextraktion (Information Extraction) aus Rechnungen führte~\cite{chargrid}.
Chargrid baut auf den Bounding-Boxen und Koordinaten der \gls{OCR}-Modelle auf und verwendet diese, um das Layout des Dokuments zu modellieren.
...
Xu et al.\ erkannten, dass bisherige Lösungen sich fast ausschließlich auf Text konzentrierten, während das Layout eines Dokuments eine entscheidende Rolle für das Verständnis spielt.
...
Die aktuellen VLMs wie das Pixtral-12B und die Qwen-2.5-VL-Familie führen diese beiden Ansätze zusammen.
Sie vereinen die Image-Encoder mit dem Wissen des LLMs und können visuelle und textuelle Merkmale verbinden.
Anknüpfend an die Evolution der Document AI beschäftigt sich diese Arbeit damit, ob ein solches generalistisches VLM die auf \gls{OCR}-Modellen beruhende Pipeline ersetzen kann.
