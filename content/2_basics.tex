...
Als Ergebnis der ersten Stufe werden dann eine PDF/A (eine archivtaugliche, unveränderbare PDF) und der extrahierte Text zurückgeliefert.
...
Um eines der \glspl{VLM} domänenspezifisch anzupassen, wurde ein \gls{PEFT} angestrebt.
Hierbei werden im Gegensatz zu einem Full-Finetuning nicht alle Modellparameter angepasst, was den Ressourcenverbrauch senkt.
...
Eine der bekanntesten Methoden im Bereich des \gls{PEFT} ist die Low-Rank Adaptation (\gls{LoRA}).
Anstatt das gesamte Modell neu zu trainieren, friert \gls{LoRA} die Gewichte ein und trainiert stattdessen Matrizen eines niedrigeren Rangs.
Diese werden nach dem Training mit der ursprünglichen Gewichtsmatrix verbunden.
